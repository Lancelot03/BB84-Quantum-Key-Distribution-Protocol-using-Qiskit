...
def test_noisy_channel_impact():
    ...
    # Wait, depolarizing noise:
    # 0.2 probability of flip.
    # If flip is X, Y, or Z (equal probability 1/3 each):
    # If Alice sent |0> (Z-basis):
    #   X flip -> |1> (Error)
    #   Y flip -> i|1> (Error)
    #   Z flip -> |0> (No error)
    # So 2/3 of 0.2 probability should be errors if bases match?
    # Actually for X basis:
    #   X flip -> |+> (No error)
    #   Y flip -> -i|-> (Error)
    #   Z flip -> |-> (Error)
    # Also 2/3.
    # So expected QBER = noise_level * 2/3 = 0.2 * 0.66 = 0.133.
    # The assertion 0.1 < qber < 0.3 still holds.
