...
from core.attacks import InterceptResend, NoisyChannel, PhotonNumberSplitting
...
def test_photon_number_splitting():
    protocol = BB84Protocol()
    ...
    # PNS should introduce NO errors (QBER = 0)
    assert qber == 0
    assert info['info_gain'] == 0.25

def test_intercept_resend_partial():
    protocol = BB84Protocol()
    intercept_prob = 0.5
    attack = InterceptResend(intercept_prob)
    backend = AerSimulator()
    n = 200
    ...
    # Intercept-Resend with 50% prob should introduce ~12.5% QBER (0.5 * 0.25)
    # Range is wider due to randomness
    assert 0.05 < qber < 0.25
    assert info['info_gain'] == 0.25 # 0.5 * 0.5
