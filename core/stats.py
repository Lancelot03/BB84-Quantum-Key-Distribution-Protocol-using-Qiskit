...
def generate_error_report(alice_bits, bob_results, alice_bases, bob_bases, sifted_alice, sifted_bob, qber):
    ...
    # Estimating Eve's information gain based on QBER
    # For Intercept-Resend, Eve gains about 0.5 bits per qubit while introducing 0.25 QBER
    eve_info_gain = min(1.0, (qber / 0.25) * 0.5) if qber > 0 else 0
