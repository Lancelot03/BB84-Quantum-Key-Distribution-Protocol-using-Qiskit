from core.stats import calculate_qber, analyze_security, generate_error_report

class SimulationEngine:
    def run(self, protocol, n, attack=None, backend=None, callback=None):
        """
        Orchestrates the QKD simulation.
        """
        def log(msg):
            if callback:
                try:
                    callback(msg)
                except:
                    pass

        log(f"Initializing {protocol.name} protocol...")

        # 1. Alice generates bits and bases
        alice_bits = protocol.generate_bits(n)
        if protocol.name == "BB84":
            alice_bases = protocol.generate_bases(n)
        else:
            alice_bases = ["B92"] * n

        # 2. Alice encodes
        log("Encoding bits into quantum states...")
        encoded_qubits = protocol.encode(alice_bits, alice_bases)

        # 3. Eve Attacks (Optional)
        if attack:
            log(f"Eavesdropper applying {attack.__class__.__name__} attack...")
            intercepted_qubits = attack.apply(encoded_qubits, backend)
        else:
            intercepted_qubits = encoded_qubits

        # 4. Bob Measures
        log("Bob performing measurements...")
        bob_bases = protocol.generate_bases(n)
        bob_results = protocol.measure(intercepted_qubits, bob_bases, backend)

        # 5. Sifting
        log("Sifting keys...")
        key_a, key_b, sifted_indices = protocol.sift(alice_bases, bob_bases, alice_bits, bob_results)

        # 6. Error Analysis
        log("Analyzing security and QBER...")
        qber = calculate_qber(key_a, key_b)
        is_secure, security_status = analyze_security(qber)
        report = generate_error_report(alice_bits, bob_results, alice_bases, bob_bases, key_a, key_b, qber=qber)

        log("Simulation complete.")

        return {
            "protocol_name": protocol.name,
            "alice_bits": alice_bits,
            "alice_bases": alice_bases,
            "bob_bases": bob_bases,
            "bob_results": bob_results,
            "key_a": key_a,
            "key_b": key_b,
            "indices": sifted_indices,
            "qber": qber,
            "is_secure": is_secure,
            "security_status": security_status,
            "report": report,
            "circuits": encoded_qubits
        }
