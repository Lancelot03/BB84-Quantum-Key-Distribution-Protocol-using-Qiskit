...
with tab1:
    ...
    if st.button("Run Simulation"):
        ...
        with st.status("Initializing Quantum Simulation...") as status:
...
with tab2:
    ...
    with visual_tab:
        ...
        # Dynamically generate Bloch coordinates for standard BB84 states
        bb84 = BB84Protocol()
        states = [
            {"name": "|0⟩ (Z-basis)", "bit": 0, "basis": "Z"},
            {"name": "|1⟩ (Z-basis)", "bit": 1, "basis": "Z"},
            {"name": "|+⟩ (X-basis)", "bit": 0, "basis": "X"},
            {"name": "|-⟩ (X-basis)", "bit": 1, "basis": "X"},
        ]

        cols = st.columns(4)
        for i, state_info in enumerate(states):
            with cols[i]:
                st.markdown(f"**State {state_info['name']}**")
                qc = bb84.encode([state_info['bit']], [state_info['basis']])[0]
                coords = get_bloch_coordinates(qc)
