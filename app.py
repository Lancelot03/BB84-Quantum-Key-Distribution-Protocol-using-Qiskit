...
with tab1:
    ...
    if st.button("Run Simulation"):
        ...
        if protocol_choice == "BB84":
            ...
            alice_bits = protocol.generate_bits(n)
            alice_bases = ["B92"] * n # Alice doesn't choose bases in B92
        ...
        if viz_data['report']:
            ...
            col1.metric("Efficiency", f"{viz_data['report']['basis_match_efficiency']:.1f}%")
            if protocol_choice == "BB84":
        ...
        st.subheader("🔍 Basis Matching / Selection")
        if protocol_choice == "BB84":
            basis_matching_visual(viz_data['alice_bases'][:20], viz_data['bob_bases'][:20])
        else:
            # For B92, Alice's basis is fixed, Bob selects
            basis_matching_visual(["B92"] * 20, viz_data['bob_bases'][:20])
...
with tab2:
    ...
    with theory_tab:
        ...
        st.subheader("B92 Protocol")
        st.write("B92 uses only two non-orthogonal states:")
        st.latex(r"|0\rangle \text{ (representing bit 0) and } |+\rangle \text{ (representing bit 1)}")

        st.subheader("Quantum Bit Error Rate (QBER)")
        st.write("The security relies on detecting errors introduced by an eavesdropper. QBER is defined as:")
    ...
    with visual_tab:
        ...
        st.write("Visualizing common states used in QKD:")
        ...
        st.subheader("3. Live Basis Matching (Example)")
