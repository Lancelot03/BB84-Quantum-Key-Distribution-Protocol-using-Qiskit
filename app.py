...
with tab1:
    ...
    if st.button("Run Simulation"):
        ...
        if not viz_data['is_secure']:
            st.error(f"⚠️ High QBER! Potential eavesdropping detected. Eve info gain: ~{viz_data['report']['eve_info_gain']:.2f} bits/qubit")
        else:
        ...
        if viz_data['report']:
            ...
            col4.metric("Eve Info Gain", f"{viz_data['report']['eve_info_gain']:.2f}")
...
with tab2:
    ...
    with visual_tab:
        ...
        for col, (bit, basis, label) in zip([col1, col2, col3, col4], states):
            with col:
                st.markdown(f"**State {label}**")
                qc = p.encode([bit], [basis])[0]
