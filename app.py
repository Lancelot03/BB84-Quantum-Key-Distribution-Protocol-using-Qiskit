...
with tab1:
    ...
    if st.button("Run Simulation"):
        ...
        if protocol_choice == "BB84":
            ...
            # Additional results for BB84 specific visualization
            results = {
                "alice_bits": alice_bits,
                "alice_bases": alice_bases,
                "bob_bases": bob_bases,
                "bob_results": bob_results,
                "circuits": encoded_qubits[:5]
            }
        else:
            # B92 Protocol
            st.info(f"Running B92 simulation...")
            res = b92.run_b92(n, eve_present, noise_level)
            key_a, key_b = res["key_a"], res["key_b"]
            qber = res["qber"]
            alice_bases, bob_bases = res["alice_bases"], res["bob_bases"]
        ...
        st.text(f"Bob's Key:   {''.join(map(str, key_b))}")
        st.markdown(f"### ❗ QBER: `{qber * 100:.2f}%` - Status: **{security_status}**")
        ...
        # Real-time Circuit Display (Only for BB84 as it's implemented with classes)
        if protocol_choice == "BB84":
