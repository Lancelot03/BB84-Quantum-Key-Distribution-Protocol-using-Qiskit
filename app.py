...
with tab1:
    ...
    if st.button("Run Simulation"):
        ...
        status = st.status("Initializing Simulation...")
        # Streamlit st.status().update() only accepts keyword arguments (like label=msg)
        # SimulationEngine passes positional arguments, so we wrap it.
