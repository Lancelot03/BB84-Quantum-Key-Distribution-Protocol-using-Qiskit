...
with tab1:
    ...
    if st.button("Run Simulation"):
        ...
        # Initialize Protocol and Attack
        ...
        # Run Engine with st.status for real-time feedback
        engine = SimulationEngine(protocol, attack)
        with st.status("Initializing Simulation...", expanded=True) as status:
            viz_data = engine.run(n, backend=backend, callback=lambda msg: status.update(label=msg))
        ...
        with col2:
            st.subheader("🔮 Bloch Sphere (First Qubit)")
            coords = get_bloch_coordinates(viz_data['circuits'][0])
