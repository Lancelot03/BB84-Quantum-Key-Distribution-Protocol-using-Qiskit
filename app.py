...
with tab1:
    ...
    with col_b:
        if protocol_choice == "BB84":
            ...
            st.info("B92 currently supports a simplified noise-based attack model.")
            noise_level = st.slider("Channel Noise Level", 0.0, 0.5, 0.1, disabled=not eve_present)
    ...
    if st.button("Run Simulation"):
        ...
        encoded_qubits = protocol.encode(alice_bits, alice_bases if protocol_choice == "BB84" else None)

        st.info(f"Running {protocol_choice} simulation with {'Eve' if eve_present else 'no'} intervention...")
        ...
        report = generate_error_report(alice_bits, bob_results, alice_bases, bob_bases, key_a, key_b) if protocol_choice == "BB84" else None
        ...
        st.subheader(f"🛠️ {protocol_choice} Quantum Circuit (First Qubit)")
        st.pyplot(draw_circuit_visual(viz_data['encoded_qubits'][0]))

        with st.expander("🛠️ Quantum Circuit Preview (First 5 Qubits)"):
            for i in range(min(5, n)):
                label = f"Qubit {i} (Basis: {viz_data['alice_bases'][i]}, Bit: {viz_data['alice_bits'][i]})" if protocol_choice == "BB84" else f"Qubit {i} (Bit: {viz_data['alice_bits'][i]})"
                st.write(label)
