...
def plot_bit_differences(key_a, key_b):
    if not key_a or not key_b:
        ...
        ax.text(0.5, 0.5, 'No sifted keys to compare', horizontalalignment='center', verticalalignment='center')
        return fig
    diffs = [int(a != b) for a, b in zip(key_a, key_b)]
    fig, ax = plt.subplots(figsize=(10, 2))
    ax.plot(diffs, marker='o', color='red', linestyle='-', label='Mismatch')
...
with tab1:
    ...
    if st.button("Run Simulation"):
        ...
        if viz_data['report'] and protocol_choice == "BB84":
...
with tab2:
    ...
    with theory_tab:
        ...
        st.write("B92 uses two non-orthogonal states:")
        st.latex(r"|0\rangle \quad \text{and} \quad |+\rangle")
    ...
    with visual_tab:
        ...
        st.subheader("3. Live Basis Matching (BB84)")
