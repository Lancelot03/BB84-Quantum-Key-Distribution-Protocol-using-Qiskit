...
with tab1:
    ...
    if st.button("Run Simulation"):
        ...
        # Report generation (BB84 has detailed report, B92 might be simpler)
        if protocol_choice == "BB84":
            report = generate_error_report(alice_bits, bob_results, alice_bases, bob_bases, key_a, key_b)
        else:
            report = {
                "total_qubits": n,
                "sifted_length": len(key_a),
                "basis_match_efficiency": (len(key_a) / n) * 100 if n > 0 else 0,
                "qber": qber,
                "z_error_rate": 0, # Not applicable for B92 in same way
                "x_error_rate": 0,
                "z_total": 0,
                "x_total": 0
            }
        ...
        with st.expander("🛠️ Quantum Circuit Preview (First 5 Qubits)"):
            for i in range(min(5, n)):
                label = f" (Basis: {viz_data['alice_bases'][i]}, Bit: {viz_data['alice_bits'][i]})" if protocol_choice == "BB84" else f" (Bit: {viz_data['alice_bits'][i]})"
                st.write(f"Qubit {i}{label}")
        ...
        if viz_data['report']:
            ...
            if protocol_choice == "BB84":
                ...
                col2.metric("Conclusive Results", f"{viz_data['report']['sifted_length']}")
                col3.metric("Total Qubits", f"{viz_data['report']['total_qubits']}")
        ...
        if len(viz_data['key_a']) > 0:
            st.pyplot(plot_bit_differences(viz_data['key_a'], viz_data['key_b']))
        else:
            st.warning("No bits in sifted key to show differences.")
