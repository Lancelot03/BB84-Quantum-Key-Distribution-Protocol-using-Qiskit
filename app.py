...
def plot_bit_differences(key_a, key_b):
    ...
    ax.plot(diffs, marker='o', color='red', linestyle='None', label='Mismatch')
...
with tab1:
    ...
    if st.button("Run Simulation"):
        ...
        # Run via SimulationEngine
        engine = SimulationEngine(protocol, attack, backend)

        with st.status("Running Quantum Simulation...", expanded=True) as status:
            results = engine.run(n, callback=lambda msg: status.update(label=msg))
            status.update(label="Simulation Complete!", state="complete", expanded=False)

        st.subheader("📬 Sifted Key Result")
        st.text(f"Alice's Key: {''.join(map(str, results['key_a']))}")
        st.text(f"Bob's Key:   {''.join(map(str, results['key_b']))}")
        st.markdown(f"### ❗ QBER: `{results['qber'] * 100:.2f}%` - Status: **{results['security_status']}**")
        ...
        st.pyplot(draw_circuit_visual(results['circuits'][0]))

        with st.expander("🛠️ Quantum Circuit Preview (First 5 Qubits)"):
            for i in range(min(5, n)):
                st.write(f"Qubit {i} (Basis: {results['alice_bases'][i]}, Bit: {results['alice_bits'][i]})")
                st.pyplot(results['circuits'][i].draw('mpl'))

        if results['report']:
            st.subheader("📊 Detailed Error Analysis")
            col1, col2, col3 = st.columns(3)
            col1.metric("Sifting Efficiency", f"{results['report']['basis_match_efficiency']:.1f}%")
            col2.metric("Z-Basis Error Rate", f"{results['report']['z_error_rate']*100:.1f}%")
            col3.metric("X-Basis Error Rate", f"{results['report']['x_error_rate']*100:.1f}%")

        st.subheader("🔍 Basis Matching")
        basis_matching_visual(results['alice_bases'][:20], results['bob_bases'][:20])

        st.subheader("🔍 Bit Differences")
        st.pyplot(plot_bit_differences(results['key_a'], results['key_b']))

        st.subheader("📊 QBER Distribution")
        st.pyplot(plot_qber_bar(results['qber']))
...
with tab2:
    ...
    with visual_tab:
        ...
        # Dynamic Bloch Sphere from selected bit/basis (for demo, just Alice's first qubit if available)
        if 'results' in locals():
            st.info(f"Visualizing Alice's first qubit: Bit {results['alice_bits'][0]} in {results['alice_bases'][0]} basis")
            coords = get_bloch_coordinates(results['circuits'][0])
            bloch_sphere(state_vector=coords, height=400)
        else:
